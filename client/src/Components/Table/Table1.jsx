...
import Alert3 from "../AlertsStudents/Alert3"
import Alert from "../AlertsStudents/Alert"
import Alert4 from "../AlertsStudents/Alert4"
...
export default () => {
    ...
    async function Remove(id) {
        let res = await axios.delete(`${config.backApi}/students/${id}`)
        fetchStudents()
        setaler(true)
        setTimeout(() => {
            setaler(false)
    ...
    async function Update() {
        if (inp1.current.value && inp2.current.value && inp3.current.value && inp4.current.value && inp5.current.value && inp6.current.value) {
            ...
            let res = await axios.put(`${config.backApi}/students/${id}`, user, axiosConfig)
            fetchStudents()
            setForm(false)
            setAler2(true)
            setTimeout(() => {
    ...
    return (
        <>
            ...
            {form && <form className="row">
                ...
                <button type='button' onClick={Update} className="btn btn-success col-5 mx-5">Submit</button>
